...
export  default {
  ...
  methods: {
    createModel: async function(model, marque) {
      const carModel = {
        model: model.value,
        marque: marque.value
      };
      console.log(JSON.stringify(carModel))
      await fetch('http://127.0.0.1:8080/carModel/new', {
        method: 'POST',
        headers: {
      ...
      <div class="flex flex-grow items-center justify-center bg-gray-900 h-full">
        <div class="max-w-full p-8 bg-gray-800 rounded-lg shadow-lg w-96 text-gray-200">
          ...
          <div>
            ...
            <input id="marque" class="flex-grow h-8 ml-4 bg-transparent focus:outline-none font-medium" placeholder="marque" type="text">
            <input id="model" class="flex-grow h-8 ml-4 bg-transparent focus:outline-none font-medium" placeholder="modèle" type="text">
            </span>
            <br>
          </div>
          <span class=" rounded-xl bg-gray-900 text-white shadow-md shadow-gray-900/10 hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none block w-full ">
          <button class="flex items-center w-full h-8 px-2 mt-2 text-sm font-medium rounded">
            ...
            ><button @click="addNewCarModel()">Add</button></span>
          </button>
          </span>
        </div>
      </div>
