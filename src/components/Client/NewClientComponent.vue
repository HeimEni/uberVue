...
export  default {
  ...
  methods: {
    createClient: async function (phoneNumber) {
      await fetch('http://127.0.0.1:8080/client/new', {
        method: 'POST',
        headers: {
    ...
    addNewClient: async function(){
      await this.createClient(document.getElementById("phoneNumber").value)
      await router.push("/")
    }
  }
}
      ...
      <div class="flex flex-grow items-center justify-center bg-gray-900 h-full">
        <div class="max-w-full p-8 bg-gray-800 rounded-lg shadow-lg w-96 text-gray-200">
          <div class="flex items-center mb-6">
            ...
            <h4 class="font-semibold ml-3 text-lg">nouveau client :</h4>
          </div>
          <div>
            ...
            <input id="phoneNumber" class="flex-grow h-8 ml-4 bg-transparent focus:outline-none font-medium" placeholder="PhoneNumber" type="text">
          ...
          <button class="flex items-center w-full h-8 px-2 mt-2 text-sm font-medium rounded">
            ...
            ><button @click="addNewClient()">Add</button></span>
          </button>
          </span>
        </div>
      </div>
