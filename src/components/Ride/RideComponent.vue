...
export default {
  ...
  methods: {
    createRide: async function (ride) {
    ...
    getRide: async function (id) {
      const response = await fetch(`http://127.0.0.1:8000/api/rides/${id}`);
      return await response.json();
    },

    getAllRides: async function () {
      const response = await fetch('http://127.0.0.1:8080/ride/');
      this.rides = await response.json();
    },

    updateRide: async function (id, ride) {
      const response = await fetch(`http://127.0.0.1:8000/api/rides/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    deleteRide: async function (id) {
      await fetch(`http://127.0.0.1:8000/api/rides/${id}`, {
        method: 'DELETE',
      });
    },
  }
      ...
      <div class="flex flex-grow items-center justify-center bg-gray-900 h-full">
        <div class="max-w-full p-8 bg-gray-800 rounded-lg shadow-lg w-96 text-gray-200">
          <div class="flex items-center mb-6">
            ...
            <h4 class="font-semibold ml-3 text-lg">Courses</h4>
          </div>
          ...
          <button class="flex items-center w-full h-8 px-2 mt-2 text-sm font-medium rounded">
            ...
            ><router-link to="/ride/new">Add new Ride</router-link></span>
          </button>
          </span>
        </div>
        <!-- Component End  -->
